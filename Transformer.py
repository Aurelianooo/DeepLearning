...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    pad_attn_mask = seq_k.data.eq(0).unsqueeze(1)  # (2, 1, 8)
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        scores = torch.matmul(Q, K.transpose(-1, -2)) / \
                 np.sqrt(d_k)  # scores : [batch_size, n_heads, len_q, len_k] (2, 8, 8, 8)
        ...
        # context: [batch_size, n_heads, len_q, d_v] (2, 8, 8, 64)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self):
        ...
        self.W_K = nn.Linear(d_model, d_k * n_heads, bias=False)  # 不然无法做点积
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        # Q: [batch_size, n_heads, len_q, d_k] (2, 8, 8, 64)
        Q = self.W_Q(input_Q).view(batch_size, -1,
                                   n_heads, d_k).transpose(1, 2)
        # K: [batch_size, n_heads, len_k, d_k] (2, 8, 8, 64) # K和V的长度一定相同，维度可以不同
        K = self.W_K(input_K).view(batch_size, -1,
                                   n_heads, d_k).transpose(1, 2)
        # V: [batch_size, n_heads, len_v(=len_k), d_v] (2, 8, 8, 64)
        V = self.W_V(input_V).view(batch_size, -1,
                                   n_heads, d_v).transpose(1, 2)

        # 因为是多头，所以mask矩阵要扩充成4维的
        # attn_mask: [batch_size, seq_len, seq_len] -> [batch_size, n_heads, seq_len, seq_len] (2, 8, 8, 8)
        attn_mask = attn_mask.unsqueeze(1).repeat(1, n_heads, 1, 1)

        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
        context, attn = ScaledDotProductAttention()(Q, K, V, attn_mask)
        # 下面将不同头的输出向量拼接在一起
        # context: [batch_size, n_heads, len_q, d_v] -> [batch_size, len_q, n_heads * d_v] (2, 8, 512)
        ...
        # return DynamicTanh(d_model).to(device)(output + residual), attn
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # return DynamicTanh(d_model).to(device)(output + residual)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        ...
        # enc_outputs: [batch_size, src_len, d_model] (2, 8, 512)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = self.pos_emb(enc_outputs.transpose(0, 1)).transpose( 0, 1)  # [batch_size, src_len, d_model] (2, 8, 512)
        # Encoder输入序列的pad mask矩阵
        enc_self_attn_mask = get_attn_pad_mask(enc_inputs, enc_inputs)  # [batch_size, src_len, src_len] (2, 8, 8)
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask + dec_self_attn_subsequence_mask),
                                      0).to(
            device)  # [batch_size, tgt_len, tgt_len] (2, 7, 7); torch.gt比较两个矩阵的元素，大于则返回1，否则返回0
        ...
        # dec_inputs仅用于提供expand的size
        dec_enc_attn_mask = get_attn_pad_mask(
            dec_inputs, enc_inputs)  # [batc_size, tgt_len, src_len] (2, 7, 8)
...
# 这样设置就不会计算 "pad" 的损失（因为本来 "pad" 也没有意义，不需要计算）
...
# ==========================================================================================
# 预测阶段
# 测试集 enc_input, dec_input, dec_output
sentences = [['我 有 一 个 女 朋 友 P', '', '']]

enc_inputs, dec_inputs, dec_outputs = make_data(sentences)
test_loader = Data.DataLoader(
    MyDataSet(enc_inputs, dec_inputs, dec_outputs), 1, True)
